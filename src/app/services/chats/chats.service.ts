...
export class ChatsService {
  ...
  addChats(uid: string, interlocutorUID: string): void {
    ...
    let endPoint: AngularFireObject<boolean> = this.db.object(
      `/users/${uid}/chats/${interlocutorUID}`
    );
    endPoint.set(true);

    let endPoint2: AngularFireObject<boolean> = this.db.object(
      `/users/${interlocutorUID}/chats/${uid}`
    );
    endPoint2.set(true);
  }
