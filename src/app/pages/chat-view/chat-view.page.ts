...
export class ChatViewPage implements OnInit {
  ...
  constructor(
    ...
    // get chat Reference
    chatsService
      .getChatRef(this.uid, this.interlocutorUID)
      .then((chatRefPath: string) => {
    ...
    this.db
      .object(`/users/${this.interlocutorUID}`)
      .valueChanges()
      .subscribe((user: User) => {
        console.log("interlo", user);
        this.interlocutorUIDData = user;
      });
  }
  ...
  sendPicture(): void {
    this.camera
      ...
      .catch((err) =>
        this.utilService.doAlert(
          `Error`,
          `This feature only works on Mobiles.`,
          "OK"
        )
      );
  }
